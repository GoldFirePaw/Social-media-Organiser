...
export async function exportRoutes(fastify: FastifyInstance) {
  fastify.get('/export', async () => {
    ...
    const scheduledPosts = ideas.flatMap(idea => 
  ...
  fastify.post('/import', async (request, reply) => {
    try {
          ...
          const ideas = body.ideas as Array<any>
          const scheduledPostsInput = Array.isArray(body.scheduledPosts) ? (body.scheduledPosts as Array<any>) : undefined
            ...
            const result = await prisma.$transaction(async (tx) => {
              ...
              for (const idea of ideas) {
                try {
                  ...
                  if (idea.id) idMap.set(idea.id, newIdea.id)
                  importedIdeas++
                } catch {
                  skippedIdeas++
                }
              ...
              const postsSource = scheduledPostsInput && Array.isArray(scheduledPostsInput)
                ? scheduledPostsInput
                : ideas.flatMap((i: any) => (Array.isArray(i.scheduledPosts) ? i.scheduledPosts : []).map((p: any) => ({ ...p, ideaId: i.id, ideaTitle: i.title, ideaPlatform: i.platform })))
              ...
              for (const post of postsSource) {
                try {
                  ...
                  let mappedIdeaId: string | undefined = undefined
                  if (post.ideaId && idMap.get(post.ideaId)) {
                    mappedIdeaId = idMap.get(post.ideaId) as string
                  } else if (post.ideaId) {
                    const maybe = await tx.idea.findUnique({ where: { id: post.ideaId } })
                    if (maybe) mappedIdeaId = post.ideaId
                  }
                  ...
                  if (!mappedIdeaId && post.ideaTitle && post.ideaPlatform) {
                    ...
                    if (found) mappedIdeaId = found.id
                  }
          ...
          const syncResult = await prisma.$transaction(async (tx) => {
            ...
            const idMap = new Map<string, string>()
            const targetIdeaIds: string[] = []
            const keepMap = new Map<string, Set<string>>() // mappedIdeaId -> set of ISO date strings to keep
            ...
            for (const idea of ideas) {
              try {
                ...
                if (existing) {
                  ...
                  if (idea.id) idMap.set(idea.id, existing.id)
                  ...
                  if (idea.id) idMap.set(idea.id, created.id)
